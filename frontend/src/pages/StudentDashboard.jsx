...
import { getMyCourses, getCourseAssignments } from '../api/courses';
...
const StudentDashboard = () => {
  ...
  const executeConfirmation = async () => {
    ...
    try {
      ...
      if (selectedCourse) await fetchCourseAssignments(selectedCourse.id);
    } catch (err) {
        ...
        {activeTab === 'courses' && selectedCourse && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-center gap-3 mb-2">
              <button
                onClick={() => { setSelectedCourse(null); setCourseAssignments([]); }}
                ...
                {courseAssignments.map((assignment) => {
                  ...
                  return (
                    <div key={assignment.id}
                      className="bg-paper-raised dark:bg-paper-dark-raised border border-rule dark:border-rule-strong rounded-xl p-6">
                      {/* Header */}
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-grow">
                          <div className="flex items-center gap-2 mb-1">
                            <h4 className="text-lg font-serif text-ink dark:text-ink-dark">{assignment.title}</h4>
                            <span className="text-xs font-mono text-ink-faint px-2 py-0.5 border border-rule rounded">
                              {subType}
                            </span>
                          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-4 mb-4 text-xs text-ink-muted">
                        ...
                        {subType === 'group' && !isLeader && (
                          <span className="text-ink-faint italic">Waiting on group leader</span>
                        )}
                        {subType === 'group' && isLeader && (
                          <span className="text-accent font-semibold">You are the leader</span>
                        )}
                        <div className="flex gap-2 ml-auto">
                          {assignment.onedrive_link && (
                            <a href={assignment.onedrive_link} target="_blank" rel="noopener noreferrer"
                              className="px-2.5 py-1 rounded bg-accent/10 text-accent hover:bg-accent/20 transition-colors font-medium">
                              OneDrive
                            </a>
                          )}
                        </div>
                      ...
                      {/* Actions */}
                      {(subType === 'individual' || isLeader) && (status === 'pending' || status === 'rejected') && (
                      ...
                      {(subType === 'individual' || isLeader) && status === 'step1_confirmed' && (
                        <button onClick={() => handleConfirmFinal(assignment)}
                          className="w-full py-2.5 px-4 bg-accent text-paper font-semibold text-sm rounded transition-all duration-200 hover:opacity-90">
                          Confirm Final Submission
                        </button>
                      )}
                      {!isLeader && subType === 'group' && (status === 'pending' || status === 'step1_confirmed') && (
                        <div className="text-center py-2 text-ink-faint text-sm italic">
                          Only the group leader can acknowledge this assignment
                        </div>
                      )}
                      {(status === 'confirmed' || status === 'accepted') && assignment.confirmed_at && (
